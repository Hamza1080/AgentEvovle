    ...
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            className={cn(
              "cursor-pointer overflow-hidden rounded-full bg-neutral-900 shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-transform duration-200",
              hovered && "scale-125 ring-2 ring-white/80",
            )}
            style={{
              width: "36px",
              height: "36px",
...
function Scene({ markers, config, onMarkerClick, onMarkerHover }: SceneProps) {
  ...
  React.useEffect(() => {
    camera.position.set(0, 0, config.radius * 2.9);
    camera.lookAt(0, 0, 0);
    ...
    <div className={cn("relative h-[500px] w-full", className)}>
      <Canvas
        ...
        camera={{
          ...
          near: 0.1,
          far: 1000,
          position: [0, 0, mergedConfig.radius * 2.9],
        }}
        style={{
